...
# Додаємо корневу папку проекту в sys.path
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))

from src.database.db import get_db
from src.schemas.schemas_auth import UserModel, UserResponse, TokenModel
from src.schemas.user import UserSchema, TokenSchema, UserResponse, TokenSchema, RequestEmail
...
def signup(body: UserModel, db: Session = Depends(get_db)):
    ...
    role = Role.admin if not repository_users.get_all_users(db) else Role.user
    body.password = auth_service.get_password_hash(body.password)
    new_user = repository_users.create_user(body, db, role=role)
    return new_user
...
def change_user_role(admin_email: str, user_email: str, new_role: Role, db: Session = Depends(get_db)):
    try:
