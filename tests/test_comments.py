...
class TestComments(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_update_comment(self):
        ...
        result = update_comment(comment_id=comment_id, body=body, db=self.session, user_id=user_id)
        self.assertEqual(result.comment, existing_comment.comment)
    async def test_delete_comment(self):
        comment_id = 1
        existing_comment = Comment(id=comment_id, comment='Comment', user_id=1, image_id=1)
        result = delete_comment(comment_id=comment_id, db=self.session)
